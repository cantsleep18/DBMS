...
void insert_into_leaf(uint64_t table_id ,pagenum_t leaf_page_num, uint64_t key, char *value, uint16_t val_size){
    ...
    leaf.record[insertion_point].offset = leaf.record[i-1].offset;
...
void remove_entry_from_node(int64_t table_id, pagenum_t key_page_num, uint64_t key){
    ...
    file_read_page(table_id, key_page_num, (page_t*)&leaf);
        ...
        for(++i; i<leaf.num_of_keys; i++){ // need to fix , addition change? leftmost like
            ...
            leaf.record[i-1].offset = leaf.record[i].offset;
            leaf.record[i-1].size = leaf.record[i].size;
            memcpy(leaf.record[i-1].value, leaf.record[i].value, 112);
        }   

        if(leaf.is_leaf)
            num_pointer = leaf.num_of_keys;
        else
            num_pointer = leaf.num_of_keys+1;
        
        i=0;
        while(leaf.record[i].key != key)
            i++;
        for(++i; i < num_pointer; i++){
            leaf.record[i-1].key = leaf.record[i].key;
            leaf.record[i-1].offset = leaf.record[i].offset;
            leaf.record[i-1].size = leaf.record[i].size;
            memcpy(leaf.record[i-1].value, leaf.record[i].value, 112);
        ...
        if(leaf.is_leaf){
            for(i=leaf.num_of_keys; i < leaf_order-1; i++) // num check plz;;;;;;
                leaf.record[i].key = 0;
        }else{
            for(i=leaf.num_of_keys+1;i<leaf_order; i++)
                leaf.record[i].key = 0;
        }
...
void coalesce_nodes(uint64_t table_id,pagenum_t root_page_num,pagenum_t key_page_num, 
    ...
    if(neighbor_index == -1){
        file_read_page(table_id, key_page_num, (page_t*)&neighbor_page);
        file_read_page(table_id, neighbor_page_num, (page_t*)&key_page);
    }                    
    ...
    if(!key_page.is_leaf){
        ...
        memcpy(neighbor_page_leaf.record[leaf_order-1].value, key_page_leaf.record[leaf_order-1].value,112);
    }
    delete_entry(table_id, key_page.parent_page_num, k_prime, key);
}
...
void redistribute_nodes(uint64_t table_id,pagenum_t root_page_num,pagenum_t key_page_num,pagenum_t neighbor_page_num, 
    ...
    if(neighbor_index != -1){
        ...
        if(!key_page.is_leaf){
            ...
            key_page.record[k_prime_index].key = neighbor_page.record[neighbor_page.num_of_keys-1].key;
        }
        else{
